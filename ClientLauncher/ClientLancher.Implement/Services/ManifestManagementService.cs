    ...
    public class ManifestManagementService : IManifestManagementService
    {
        ...
        public async Task<ManifestResponse> CreateManifestAsync(int applicationId, ManifestCreateRequest request, string? createdBy = null)
        {
            try
            {
                ...
                var manifest = new ApplicationManifest
                {
                    ...
                    BinaryFilesJson = request.BinaryFiles.Any()
                        ? JsonSerializer.Serialize(request.BinaryFiles)
                        : null,
                    ...
                    ConfigFilesJson = request.ConfigFiles.Any()
                        ? JsonSerializer.Serialize(request.ConfigFiles)
                        : null,
        ...
        public async Task<ManifestResponse> UpdateManifestAsync(int manifestId, ManifestUpdateRequest request, string? updatedBy = null)
        {
            try
            {
                ...
                if (request.BinaryFiles != null)
                    manifest.BinaryFilesJson = request.BinaryFiles.Any()
                        ? JsonSerializer.Serialize(request.BinaryFiles)
                        : null;
                ...
                if (request.ConfigFiles != null)
                    manifest.ConfigFilesJson = request.ConfigFiles.Any()
                        ? JsonSerializer.Serialize(request.ConfigFiles)
                        : null;
                ...
                if (request.NotifyUser.HasValue)
                    manifest.NotifyUser = request.NotifyUser.Value;

                if (request.AllowSkip.HasValue)
                    manifest.AllowSkip = request.AllowSkip.Value;

                if (request.UpdateDescription != null)
                    manifest.UpdateDescription = request.UpdateDescription;
        ...
        public async Task<IEnumerable<ManifestResponse>> GetManifestHistoryAsync(int applicationId, int take = 10)
        {
            ...
            return new List<ManifestResponse>(manifests.Select(m => MapToResponse(m)));
        }

        /// <summary>
        /// Generate JSON manifest for client download
        /// </summary>
        public async Task<ManifestJsonResponse?> GenerateManifestJsonAsync(string appCode)
        {
            try
            {
                ...
                List<ManifestJsonResponse.ConfigFilePolicy> configFiles = new();
                if (!string.IsNullOrEmpty(manifest.ConfigFilesJson))
                {
                    try
                    {
                        var policies = JsonSerializer.Deserialize<List<ConfigFilePolicyRequest>>(manifest.ConfigFilesJson);
                        if (policies != null)
                        {
                            configFiles = policies.Select(p => new ManifestJsonResponse.ConfigFilePolicy
                            {
                                Name = p.Name,
                    ...
                    {
                        _logger.LogWarning(ex, "Failed to deserialize config files JSON for {AppCode}", appCode);
                    }
                }

                // Deserialize binary files
                List<string> binaryFiles = new();
                if (!string.IsNullOrEmpty(manifest.BinaryFilesJson))
                {
                    try
                    {
                        var files = JsonSerializer.Deserialize<List<string>>(manifest.BinaryFilesJson);
                        if (files != null)
                        {
                            binaryFiles = files;
                        }
                    ...
                    {
                        _logger.LogWarning(ex, "Failed to deserialize binary files JSON for {AppCode}", appCode);
                    }
                }
                ...
                var response = new ManifestJsonResponse
                {
                    ...
                    UpdatePolicy = new ManifestJsonResponse.UpdatePolicyInfo
                    {
                        ...
                        NotifyUser = manifest.NotifyUser,
                        AllowSkip = manifest.AllowSkip,
                        Description = manifest.UpdateDescription
                    }
                };
        ...
        private ManifestResponse MapToResponse(ApplicationManifest manifest, Application? app = null)
        {
            ...
            // Deserialize config files
            List<ConfigFilePolicyResponse> configFiles = new();
            if (!string.IsNullOrEmpty(manifest.ConfigFilesJson))
            {
                try
                {
                    ...
                    if (policies != null)
                    {
                        configFiles = policies.Select(p => new ConfigFilePolicyResponse
                        {
                            Name = p.Name,
                            UpdatePolicy = p.UpdatePolicy,
                            Priority = p.Priority
                ...
                {
                    _logger.LogWarning(ex, "Failed to deserialize config files for manifest {ManifestId}", manifest.Id);
                }
            }
            ...
            if (!string.IsNullOrEmpty(manifest.BinaryFilesJson))
            {
                ...
                {
                    _logger.LogWarning(ex, "Failed to deserialize binary files for manifest {ManifestId}", manifest.Id);
                }
            }
            ...
            return new ManifestResponse
            {
                ...
                PublishedAt = manifest.PublishedAt.Value,
